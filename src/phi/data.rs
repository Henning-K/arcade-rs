...
impl Rectangle {
    ...
    /// Panics if it could not be created, for example if a coordinate of a corner overflows an `i32`.
    pub fn to_sdl(self) -> SdlRect {
        // Reject negative width and height
        assert!(self.w >= 0.0 && self.h >= 0.0);

        // SdlRect::new : `(i32, i32, u32, u32) -> Result<Option<SdlRect>>`
        // That was the case in SDL2 0.13
        // In version 0.24 SdlRect#new just returns Self, not Result<Option<Self>> 
    ...
    pub fn contains(&self, rect: Rectangle) -> bool {
        ...
        xmax >= self.x && xmax <= self.x + self.x &&
